...
export const AuthContext = createContext({
    user: INITIAL_USER_STATE,
    isAuthenticated: false,
    isLoading: true, // Start true to indicate initial auth check
    authError: null,
    ...
    fetchAndUpdateUser: async () => { throw new Error("fetchAndUpdateUser function not implemented") },
    clearAuthError: () => {},
});

export const AuthProvider = ({ children }) => {
    const [persistedUser, setPersistedUser] = useLocalStorage(LOCAL_STORAGE_KEYS.AUTH_USER, INITIAL_USER_STATE);
    const [currentUser, setCurrentUser] = useState(persistedUser);
    const [isAuthenticated, setIsAuthenticated] = useState(!!persistedUser);
    const [isLoading, setIsLoading] = useState(true); // True until initial check is done
    const [authError, setAuthError] = useState(null);

    const navigate = useNavigate();
    const location = useLocation();

    // Effect to sync local state with localStorage and manage initial loading
    useEffect(() => {
        logger.debug("AuthContext: Persisted user from localStorage changed or initial load:", persistedUser);
        setCurrentUser(persistedUser);
        setIsAuthenticated(!!persistedUser);
        setIsLoading(false); // Done with initial load based on localStorage
    }, [persistedUser]);

    // Effect for handling session expiry event dispatched by apiClient
    useEffect(() => {
        const handleSessionExpired = async () => {
            logger.warn("AuthContext: 'auth_session_expired_logout' event received.");
            // Check if we are not already logged out or in the process of logging out
            // This prevents multiple logout calls if event is dispatched multiple times
            if (isAuthenticated || currentUser) { // Check against current context state
                logger.info("AuthContext: Current state is authenticated, proceeding with logout due to session expiry.");
                await performLogoutSequence(true); // Pass true to indicate forced redirect
            } else {
                logger.info("AuthContext: Session expiry event received, but user already logged out or no user in context. No action.");
            }
        };

        if (typeof window !== 'undefined') {
            window.addEventListener('auth_session_expired_logout', handleSessionExpired);
            logger.info("AuthContext: Event listener for 'auth_session_expired_logout' attached.");
        }
        return () => {
            if (typeof window !== 'undefined') {
                window.removeEventListener('auth_session_expired_logout');
                logger.info("AuthContext: Event listener for 'auth_session_expired_logout' removed.");
            }
        };
    }, [isAuthenticated, currentUser]); // Re-attach if isAuthenticated or currentUser changes, logout needs latest state

    // Internal logout sequence, to be called by logout or event handler
    const performLogoutSequence = useCallback(async (forceRedirectToLogin = false) => {
        logger.info(`AuthContext: performLogoutSequence called. Force redirect: ${forceRedirectToLogin}`);
        setIsLoading(true); // Indicate a process is happening
        setAuthError(null);
        ...
        try {
            ...
            setPersistedUser(INITIAL_USER_STATE); // Clears localStorage and triggers useEffect to update currentUser & isAuthenticated
            // setCurrentUser(INITIAL_USER_STATE); // Handled by useEffect on persistedUser
            // setIsAuthenticated(false); // Handled by useEffect on persistedUser
            ...
            if (forceRedirectToLogin) {
                logger.info('AuthContext: Redirecting to login page.');
                navigate(ROUTES.LOGIN, { 
                    replace: true, 
                    state: location.pathname !== ROUTES.LOGIN ? { from: location, sessionExpired: true } : undefined 
                });
            }
        }
    }, [setPersistedUser, navigate, location]);
    ...
    const login = useCallback(async (email, password) => {
        ...
        try {
            ...
            if (response.success && response.data.user) {
                const loggedInUser = response.data.user;
                setPersistedUser(loggedInUser); // This updates localStorage and triggers useEffect to update currentUser & isAuthenticated
                logger.info('AuthContext: Login successful', { email: loggedInUser.email, role: loggedInUser.role });
                ...
                logger.info(`AuthContext: Navigating to ${redirectTo} after login.`);
                navigate(redirectTo, { replace: true });
                return { success: true, user: loggedInUser }; // Return success and user
            } else {
                // This case is if API returns success:false but doesn't throw HTTP error
                const message = response.message || 'Login failed. Please check credentials.';
                setAuthError(message);
                throw new Error(message); // Ensure error is propagated
            }
        } catch (err) {
            const errorMessage = err.response?.data?.message || err.message || 'Login failed. Please check your credentials.';
            logger.error('AuthContext: Login error', { errorMessage, originalErrorDetails: err.response?.data || err });
            setAuthError(errorMessage);
            // Do not clear persistedUser here, as a failed login shouldn't log out an existing valid session (if any)
            throw new Error(errorMessage); // Re-throw for LoginForm to catch
        } finally {
            setIsLoading(false);
        }
    }, [setPersistedUser, navigate, location.state]);
    ...
    const register = useCallback(async (userData) => {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || err.message || 'Registration failed.';
            logger.error('AuthContext: Registration error', {errorMessage, originalErrorDetails: err.response?.data || err});
            setAuthError(errorMessage);
            throw new Error(errorMessage);
        } finally {
            setIsLoading(false);
    ...
    // This function is for explicitly refreshing user data if needed, e.g., after profile update
    // Or to validate session on app focus/re-entry.
    const fetchAndUpdateUser = useCallback(async () => {
        if (!isAuthenticated && !localStorage.getItem(LOCAL_STORAGE_KEYS.AUTH_USER)) {
             logger.debug("AuthContext: fetchAndUpdateUser - no persisted user or not authenticated, skipping /me call.");
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                logger.warn('AuthContext: Failed to refresh user data from /auth/me (API non-success). Forcing logout.', response.message);
                await performLogoutSequence(true); // Force logout and redirect
            }
        } catch (error) {
            logger.error('AuthContext: Error fetching current user data for update:', error);
            if (error.response?.status === 401 || error.response?.status === 403) {
                logger.warn('AuthContext: Received 401/403 on /auth/me. Forcing logout.');
                await performLogoutSequence(true); // Force logout and redirect
            }
            // If other error, don't necessarily logout, could be network issue
        }
    }, [isAuthenticated, setPersistedUser, performLogoutSequence]); // Changed logout to performLogoutSequence

    // Initial check on app load to validate persisted session
    useEffect(() => {
        const validateSession = async () => {
            if (localStorage.getItem(LOCAL_STORAGE_KEYS.AUTH_USER)) { // Check if there's a user in LS
                logger.info("AuthContext: Persisted user found. Validating session with backend via /auth/me.");
                await fetchAndUpdateUser(); // This will validate token and update user or logout
            } else {
                setIsLoading(false); // No user in LS, initial check complete, not loading.
            }
        };
        validateSession();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []); // Run only once on initial mount to validate any existing session
    ...
    const contextValue = useMemo(() => ({
        ...
        error: authError,
        login,
        logout: () => performLogoutSequence(true), // Expose a simple logout that always redirects
        register,
        fetchAndUpdateUser,
        clearAuthError: () => setAuthError(null)
    }), [currentUser, isAuthenticated, isLoading, authError, login, performLogoutSequence, register, fetchAndUpdateUser]);
